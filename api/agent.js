...
export default async function handler(req, res) {
  ...
  const allTools = [
    ...
    { type: "function", function: { name: "find_nearby", description: "Find places near the user's last shared Telegram location (e.g. 'coffee shops near me').",
      parameters: { type: "object", properties: { query: { type: "string" } }, required: ["query"] } } },
    { type: "function", function: { name: "react_to_message", description: "React with one emoji when it's genuinely the right response — something is funny, impressive, relatable, or a short exchange doesn't need a full reply. This is a real reaction, not just an efficiency shortcut for short messages.",
    ...
    { type: "function", function: { name: "get_current_datetime", description: "Get the real current date, time, and day of the week. ALWAYS use this for any question about today's date, the current day of the week, or what time it is — never guess from memory.",
      parameters: { type: "object", properties: {}, required: [] } } },
    { type: "function", function: { name: "get_ethiopian_date", description: "Get today's date converted to the Ethiopian calendar. Use whenever the user asks for the Ethiopian calendar date, Habesha date, or similar.",
      parameters: { type: "object", properties: {}, required: [] } } },
  ...
  async function listLinks(url) {
    try {
      ...
      $('a[href]').each((i, el) => {
        ...
        if (!text || !href || href.startsWith('#') || href.startsWith('javascript:') || href.startsWith('mailto:')) return;
        try {
  ...
  async function getNews(query) {
    try {
      ...
      const data = await r.json();
      const top = (data.news || []).slice(0, 5).map(n => `${n.title} (${n.date || 'recent'}) — ${n.source || ''} — ${n.link || ''}`).join('\n');
  ...
  // Deterministic Gregorian -> Ethiopian calendar conversion (Julian Day
  // Number based), verified against known reference dates (Ethiopian New
  // Year 2016/2017/2019 all matched exactly) before shipping this.
  async function getEthiopianDate() {
    const months = ['Meskerem', 'Tikimt', 'Hidar', 'Tahsas', 'Tir', 'Yekatit', 'Megabit', 'Miazia', 'Ginbot', 'Sene', 'Hamle', 'Nehase', 'Pagume'];

    const nowLocalStr = new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' });
    const nowLocal = new Date(nowLocalStr);
    const gYear = nowLocal.getFullYear();
    const gMonth = nowLocal.getMonth() + 1;
    const gDay = nowLocal.getDate();
    ...
    const jdn = toJDN(gYear, gMonth, gDay);
    const r = (jdn - JD_EPOCH_OFFSET) % 1461;
    const n = (r % 365) + 365 * Math.floor(r / 1460);
    const eYear = 4 * Math.floor((jdn - JD_EPOCH_OFFSET) / 1461) + Math.floor(r / 365) - Math.floor(r / 1460);
    const eMonthIndex = Math.floor(n / 30);
    const eDay = (n % 30) + 1;

    return `Today's Ethiopian calendar date: ${eDay} ${months[eMonthIndex]} ${eYear} E.C.`;
  }
    ...
    for (let i = 0; i < 4; i++) { // max 4 tool-call loops — balance between rate-limit safety and letting genuine multi-step tasks finish
      ...
      if (choice.tool_calls) {
        for (const call of choice.tool_calls) {
          ...
          else if (call.function.name === 'get_ethiopian_date') result = await getEthiopianDate();
